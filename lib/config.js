...
Config.prototype.resolvePath = function(file){
    var cwd = this.get('cwd')
    if (cwd) return path.join(cwd, file)
    else return file
}
...
Config.prototype.getFileSet = function(want, dontWant, callback){
    ...
    async.reduce(want, [], function(allThatIWant, patternEntry, next){
        ...
        fileset(path.join(cwd, pattern), path.join(cwd, dontWant), function(err, files){
...
Config.prototype.getTemplateData = function(cb){
    ...
    this.getServeFiles(function(err, files){
        ret.serve_files = unCwdify(files)
