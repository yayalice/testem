    ...
    , initServer: function(){
        ...
        function renderDefaultTestPage(req, res){
            ...
            if (test_page){
                ...
                function render(err, files){
                    ...
                    res.render(__dirname + '/../views/' + runnerPage, {
                        locals: {
                          styles: css_files,
                          scripts: unCwdify(files)
                        }
                    })

                    function unCwdify(files){
                        var cwd = config.get('cwd')
                        if (!cwd) return files
                        return files.map(function(file){
                            var src = file.src
                            if (src.substring(0, cwd.length) === cwd)
                                src = src.substring(cwd.length)
                            return {src: src, attrs: file.attrs}
                        })
                    }
                }
            }
        ...
        exp.get('/testem.js', function(req, res){
            ...
            async.forEachSeries(files, function(file, done){
                fs.readFile(__direname + '/../public/testem/' + file, function(err, data){
                    if (err){
        ...
        function serveStaticFile(uri, req, res){
            ...
            var cwd = config.get('cwd') || process.cwd()
            var filePath = path.resolve(cwd, uri)
